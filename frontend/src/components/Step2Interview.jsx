...
function Step2Interview({ interviewData, onFinish }) {
  ...
  // Refs for values async functions must read without stale closures
  ...
  const speakText = (text) => {
    return new Promise((resolve) => {
      ...
      utterance.onend = () => {
        ...
        // Only restart mic if user hasn't submitted yet and mic is enabled
        if (isMicOnRef.current && !timerStoppedRef.current) {
          startMic();
        }
  ...
  useEffect(() => {
    if (!isIntroPhase && currentQuestion) {
      ...
      // Restore mic for each new question
      setIsMicOn(true);
      isMicOnRef.current = true;
    }
  }, [currentIndex, isIntroPhase]);

  // Timer — only runs when question is ready
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setTimeLeft((prev) => {
        // If submit was clicked, timerStoppedRef is already true — freeze immediately
  ...
  }, [isIntroPhase, currentIndex, isQuestionReady]);

  // When timer naturally hits 0: stop mic, reveal pending feedback or auto-submit
  useEffect(() => {
    if (isIntroPhase) return;
    if (!currentQuestion) return;
    if (timeLeft !== 0) return;
    // If submit button was already clicked, submitAnswer already handled everything
    if (timerStoppedRef.current) return;

    timerStoppedRef.current = true;

    // Mute mic when time runs out naturally
    stopMic();
    isMicOnRef.current = false;
    setIsMicOn(false);

    // If API already returned while timer was running, reveal the feedback now
    if (pendingFeedbackRef.current && !feedback) {
      setFeedback(pendingFeedbackRef.current);
      speakText(pendingFeedbackRef.current);
      return;
    }

    // Otherwise auto-submit
  ...
  const submitAnswer = async () => {
    // Ref-based guard — synchronous, prevents double-submit on first click
    if (isSubmittingRef.current) return;
    isSubmittingRef.current = true;
    setIsSubmitting(true);

    // Stop timer immediately — ref is synchronous so interval reads it on next tick
    timerStoppedRef.current = true;

    // Mute mic immediately on submit
    stopMic();
    isMicOnRef.current = false;
    setIsMicOn(false);
    ...
    try {
      ...
      pendingFeedbackRef.current = feedbackText;

      // timerStoppedRef is always true here — reveal feedback immediately on first click
      ...
      >
        ...
        >
          ...
          {subtitle && (
            ...
            >
              <p
                className="text-gray-700 text-sm sm:text-base font-medium text-center
              leading-relaxed"
              >
                {subtitle}
              </p>
        ...
        <div className="flex-1 flex flex-col p-4 sm:p-6 md:p-8 relative">
          ...
          {!feedback ? (
            ...
            rounded-full bg-black text-white shadow-lg disabled:opacity-50"
              >
                {isMicOn && !isAIPlaying ? (
            ...
            py-3 sm:py-4 rounded-2xl shadow-lg hover:opacity-90 transition font-semibold disabled:opacity-50"
              >
            ...
            >
              ...
              <button
                onClick={handleNext}
                className="w-full bg-linear-to-r from-emerald-600 to-teal-500 text-white 
              py-3 rounded-xl shadow-md hover:opacity-90 transition flex items-center justify-center gap-1"
              >
